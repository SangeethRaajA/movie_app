...
import { getSeriesList } from "@/lib/tmdb";
...
export default async function SeriesPage({ searchParams }: SeriesPageProps) {
  ...
  const response = await getSeriesList(category, page);
    ...
    <main className="min-h-screen bg-[radial-gradient(circle_at_top,_#11223d_0%,_#020617_60%,_#01020b_100%)] text-white">
      <section className="mx-auto flex max-w-7xl flex-col gap-8 px-6 py-10 sm:px-8 lg:px-10">
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            <p className="text-sm uppercase tracking-[0.3em] text-cyan-300">Series</p>
            <h1 className="text-3xl font-semibold text-white">Binge-worthy television</h1>
          </div>
          <div className="flex flex-wrap gap-2">
            {categories.map((item) => (
              <Link
                key={item.key}
                href={`/series?category=${item.key}`}
        ...
        <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
          {series.map((item) => (
            <MovieCard key={item.id} item={item} href={`/tv/${item.id}`} type="tv" />
          ))}
        </div>
        ...
        {totalPages > 1 ? (
          <div className="flex flex-wrap items-center justify-center gap-2">
            {page > 1 ? (
              <Link href={`/series?category=${category}&page=${page - 1}`} className="rounded-full border border-white/10 px-3 py-2 text-sm text-zinc-300 hover:border-cyan-400/30 hover:text-white">
                Previous
              </Link>
            ) : null}
            {[...Array(Math.min(5, totalPages))].map((_, index) => {
              ...
              return (
                <Link
                  key={pageNumber}
                  href={`/series?category=${category}&page=${pageNumber}`}
